...
@router_budget.post(
    "/",
    status_code=201,
    summary="Set monthly budget limit",
    description="Creates the user's monthly budget limit. Fails if one already exists — use PUT to update.",
)
def create_budget(
    budget: BudgetInput,
    ...
    """
    existing = (
        db.query(Budget).filter(Budget.user_id == current_user["user_id"]).first()
    )
    if existing:
        raise HTTPException(
            status_code=400, detail="Budget already exists. Use PUT to update it."
        )

    new_budget = Budget(
        user_id=current_user["user_id"], monthly_limit=budget.monthly_limit
    )
    db.add(new_budget)
    db.commit()
...
@router_budget.get(
    "/",
    summary="Get the current monthly budget limit",
    description="Returns the authenticated user's monthly budget limit.",
)
def get_budget(
...
@router_budget.put(
    "/",
    summary="Update the monthly budget limit",
    description="Updates the authenticated user's monthly budget limit. Returns 404 if none exists yet.",
)
def update_budget(
    budget: BudgetInput,
    current_user: dict = Depends(get_current_user),
    ...
    if not existing:
        raise HTTPException(
            status_code=404, detail="No budget set yet. Use POST to create one."
        )
...
@router_budget.get(
    "/watchdog",
    summary="Budget watchdog status",
    description="Returns current-month total spending against the monthly limit, for the dashboard's Spending Velocity card.",
)
def get_budget_watchdog(
    current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    ...
    spending_percent = (
        round((total_spent / monthly_limit) * 100, 2) if monthly_limit > 0 else 0
    )
